...
@router.get('')
async def get_estatistikak(stat: EstatistikaTypeEnum,
    ...
    if stat_type == 'cumulative':
        result = await asyncio.to_thread(get_culumative_stats, league, year, team, category)
        ...
        if team:
            ...
            result = await asyncio.to_thread(get_points_per_race, league, year, category)
    elif stat_type == 'rank':
        ...
        result = await asyncio.to_thread(get_incorporations, league, year, team)
    return result
